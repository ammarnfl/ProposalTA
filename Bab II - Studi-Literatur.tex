...
\begin{enumerate}
    \item \textcite{putra2022monitoring} mengembangkan sistem \textit{monitoring} gizi siswa berbasis \textit{mobile} yang memungkinkan guru melaporkan data konsumsi makanan secara \textit{real-time}.
    \item \textcite{rahmawati2021decisiontree} mengusulkan sistem deteksi dini kasus gizi buruk menggunakan algoritma \textit{Decision Tree} dengan tingkat akurasi mencapai 89\%.
    \item \textcite{santoso2020iot} merancang sistem evaluasi kualitas makanan di sekolah berbasis \textit{Internet of Things} (IoT) yang mengirimkan data suhu dan kesegaran makanan ke \textit{server} pusat.
\end{enumerate}
